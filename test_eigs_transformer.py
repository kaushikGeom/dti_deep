...
modelname="Eigs_transformer_lambdas_gpu_opt=Eigs_128x128_dirs65DTI_bs16_bpe100_snr100_lr_0.001"
...
S_invivo=torch.tensor(data[:,:,20:36,:]).float()
#S_invivo=torch.tensor(data).float()

S_invivo=map_to_range(S_invivo, 0.,1 )
...
print(S_invivo[None,:,:,:16,:].shape)
#exit()
model.eval()
with torch.inference_mode(): 
            
                #D_est, _ =model(S=S_invivo.permute(2,3,0,1), gradients=torch.tensor(bvecs) )
                eigvals_est =model(S_invivo[:,:,:16,:].permute(2,0,1,3).to(device))
                eigvals_est=eigvals_est.permute(1,2,0,3)
                
                eigvals_est=eigvals_est.squeeze()
                ...
                eigvals_est=eigvals_est.cpu()*mask[:,:,:16, None]
                eigvals_gt=eigvals_gt*mask[:,:,:16, None]
                ...
                #eigvals_est[...,0]=map_to_range(eigvals_est[...,0], eigvals_gt[...,0].min(), eigvals_gt[...,0].max())
                #eigvals_est[...,1]=map_to_range(eigvals_est[...,1], eigvals_gt[...,1].min(), eigvals_gt[...,1].max())
                #eigvals_est[...,2]=map_to_range(eigvals_est[...,2], eigvals_gt[...,2].min(), eigvals_gt[...,2].max())
...
# Row 1: Estimations
im5 = axs[0, 0].imshow(md_est, cmap='viridis', vmax= 0.90*md_est.max())
im6 = axs[0, 1].imshow(fa_est, cmap='viridis', vmax= 0.90*fa_est.max())
im7 = axs[0, 2].imshow(ad_est, cmap='viridis', vmax= 0.90*ad_est.max())
im8 = axs[0, 3].imshow(rd_est, cmap='viridis', vmax= 0.90*rd_est.max())

im9  = axs[1, 0].imshow(md_fit, cmap='viridis', vmax= 0.90*md_fit.max())
im10 = axs[1, 1].imshow(fa_fit, cmap='viridis', vmax= 0.90*fa_fit.max())
im11 = axs[1, 2].imshow(ad_fit, cmap='viridis', vmax= 0.90*ad_fit.max())
im12 = axs[1, 3].imshow(rd_fit, cmap='viridis', vmax= 0.90*rd_fit.max())
